...
namespace ThreeDISevenZeroR.SensorKit
{
    ...
    public static class PhysicsSensorUtils
    {
        ...
        public static void GetCapsulePoints(Vector3 origin, Quaternion rotation, 
            float width, Vector3 scale, AxisDirection direction,
            out Vector3 p1, out Vector3 p2)
        {
            ...
            switch (direction)
            {
                case AxisDirection.XAxis: axisOffset = new Vector3(Mathf.Abs(width * scale.x), 0, 0); break;
                case AxisDirection.YAxis: axisOffset = new Vector3(0, Mathf.Abs(width * scale.y), 0); break;
                case AxisDirection.ZAxis: axisOffset = new Vector3(0, 0, Mathf.Abs(width * scale.y)); break;
                default: axisOffset = Vector3.zero; break;
            }

            var offset = rotation * axisOffset;
            p1 = origin + offset;
            p2 = origin - offset;
        }
            ...
            float width, float radius, AxisDirection direction)
        {
            ...
            GetCapsulePoints(center, rotation, width, scale, direction, out p1, out p2);
